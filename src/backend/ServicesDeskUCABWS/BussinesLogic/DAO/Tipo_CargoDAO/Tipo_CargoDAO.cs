...
namespace ServicesDeskUCABWS.BussinesLogic.DAO.Tipo_CargoDAO
{
    public class Tipo_CargoDAO : ITipo_CargoDAO
    {
        ...
        public void AgregarCargoATipoEstadoCargo(Tipo_Cargo Tipo_Cargo)
        {
            var listaDepartamentos = _dataContext.Departamentos.ToList();

            var listacargos = new List<Cargo>();

            foreach (var departamento in listaDepartamentos)
            {
                listacargos.Add(new Cargo(Tipo_Cargo.nombre + " " + departamento.nombre, Tipo_Cargo.descripcion)
                {
                    id = Guid.NewGuid(),
        ...
        {
            var listaTipo = _dataContext.Cargos.Where(x => x.Tipo_Cargo.id == tipoId);
